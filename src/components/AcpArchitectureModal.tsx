...
export const AcpArchitectureModal: React.FC<AcpArchitectureModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-surface border border-border rounded-2xl w-full max-w-4xl text-fg overflow-hidden shadow-2xl flex flex-col max-h-[92vh]">
        {/* Header */}
        <div className="p-4 border-b border-border flex items-center justify-between bg-surface-subtle">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="font-bold text-base text-fg flex items-center gap-2">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 font-mono">
                  ACP v1.0 规范解析
                </span>
              </h2>
          ...
          <button
            onClick={onClose}
            className="text-fg-muted hover:text-fg transition-colors cursor-pointer p-1"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-5 overflow-y-auto space-y-5 text-xs font-sans">
          ...
          <div className="p-3.5 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-fg space-y-1.5">
            <div className="flex items-center gap-2 font-bold text-emerald-600 dark:text-emerald-300 text-sm">
              <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0" />
              <span>核心结论：是的，不仅可以使用，而且三者均具备清晰的隔离与协同机制！</span>
            </div>
            <p className="text-[11px] text-fg-secondary leading-relaxed pl-6">
          ...
          <div className="p-4 rounded-lg bg-surface-subtle border border-border space-y-2">
            <div className="text-[11px] font-mono text-fg-muted font-semibold uppercase flex items-center justify-between">
              <span>Block Buzz 与 ACP 多 Agent 协同架构拓扑图</span>
              <span className="text-emerald-500">Buzz (Nostr) ↔ ACP Harness ↔ Agent Runtime</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 font-mono text-[11px] pt-1">
              ...
              <div className="p-3 rounded bg-surface border border-border space-y-1.5">
                ...
                <div className="text-[10px] text-fg-muted space-y-1">
                  ...
                  <div>• 传递工作区根路径: <code className="text-emerald-500">cwd</code></div>
                  <div>• 分发会话指令: <code className="text-emerald-500">session/prompt</code></div>
          ...
          <div className="space-y-3">
            <h3 className="font-bold text-sm text-fg border-b border-border pb-1.5">
              三大关键要素详细解析与代码规范
            </h3>
            ...
            <div className="p-3.5 rounded-lg bg-surface-subtle border border-border space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10px] font-mono">
                <div className="p-2.5 rounded bg-surface border border-border">
                  <div className="text-emerald-600 dark:text-emerald-300 font-bold mb-1">A. Agent 自带的私有长期记忆</div>
                ...
                <div className="p-2.5 rounded bg-surface border border-border">
                  <div className="text-blue-600 dark:text-blue-300 font-bold mb-1">B. Buzz 团队房间讨论短期记忆</div>
                  <div className="text-fg-muted space-y-0.5">
                    <div>• 存储在 Buzz 的 Nostr Relay 日志中</div>
                    <div>• 属于团队多人可见的公共历史</div>
                    <div>• 每次调用时通过 ACP <code className="text-blue-500">session/prompt</code> 注入</div>
            ...
            <div className="p-3.5 rounded-lg bg-surface-subtle border border-border space-y-2">
              ...
              <p className="text-[11px] text-fg-secondary leading-relaxed">
                当客户端拉取 Agent 参与讨论时，会在 ACP 的 <code className="text-blue-500 font-mono">session/new</code> 协议中传递宿主指定的仓库根目录：
              </p>
              <pre className="p-2 rounded bg-surface text-[10px] font-mono text-fg-secondary overflow-x-auto border border-border">
{`// ACP session/new JSON-RPC 报文
{
  ...
  "params": {
    ...
    "permissionMode": "full_read_write" // 支持只读或完整读写
              ...
              <p className="text-[11px] text-fg-secondary leading-relaxed">
                Agent 进程（如 <code className="text-blue-500 font-mono">buzz-agent</code>）可以直接在本地该目录下读取代码、运行 <code className="text-fg font-mono">git diff</code>、创建分支或发起提交。如果是远程 Agent，则可以通过 ACP 的 <code className="text-blue-500 font-mono">workspace/readFile</code> 和 <code className="text-blue-500 font-mono">workspace/applyEdit</code> RPC 方法请求客户端代理操作。
              </p>
            </div>
            ...
            <div className="p-3.5 rounded-lg bg-surface-subtle border border-border space-y-2">
              ...
              <p className="text-[11px] text-fg-secondary leading-relaxed">
                Block 在开源 Buzz 时，配套开源了 <strong className="text-purple-600 dark:text-purple-300">buzz-dev-mcp</strong>（基于 MCP 标准的开发工具服务）。ACP 架构天然解耦了技能与通信协议：
              </p>
              <ul className="list-disc list-inside space-y-1 text-fg-muted text-[10px]">
                <li>
                  <strong className="text-fg">Agent 自带工具集：</strong> Agent 作为 MCP Client，可在启动时自动连接本地配置的各种 MCP Servers（如数据库查询、代码分析工具、外部 API 接口）。
                </li>
                <li>
                  <strong className="text-fg">双向协议协商：</strong> 在 ACP 的 <code className="text-purple-500 font-mono">initialize</code> 阶段，Agent 与 Client 会宣告双方支持的 Tools 与 Prompts。Agent 可以直接在其响应中发起工具调用（Tool Calls），完成运行测试或搜索后向讨论房间输出最终结论。
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-border">
            ...
            <button
              onClick={onClose}
              className="px-4 py-1.5 rounded bg-accent text-accent-fg hover:opacity-90 font-medium text-xs transition-colors cursor-pointer shadow-xs"
            >
              返回工作台体验
            </button>
          </div>
